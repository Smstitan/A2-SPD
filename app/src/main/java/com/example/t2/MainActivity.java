...
public class MainActivity extends AppCompatActivity implements OnCellClickListener {
    public static final long TIMER_LENGTH = 599000L;    // 999 seconds in milliseconds
    ...
    private TextView smiley, timer, mark, marksLeft;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        marksLeft = findViewById(R.id.activity_main_marksleft);

        mineSweeperGame = new MineSweeperGame(GRID_SIZE, BOMB_COUNT);
        marksLeft.setText(String.format("%03d", mineSweeperGame.getNumberBombs() - mineSweeperGame.getMarkCount()));
        ...
        mark = findViewById(R.id.activity_main_mark);
        mark.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                mineSweeperGame.toggleMode();
                if (mineSweeperGame.isMarkMode()) {
                    GradientDrawable border = new GradientDrawable();
                    border.setColor(0xFFFFFFFF);
                    border.setStroke(1, 0xFF000000);
                    mark.setBackground(border);
                } else {
                    GradientDrawable border = new GradientDrawable();
                    border.setColor(0xFFFFFFFF);
                    mark.setBackground(border);
